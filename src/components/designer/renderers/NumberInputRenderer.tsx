...
export function NumberInputRenderer({
  ...
  const minVal = readProp<number | ''>(component, 'min', '');
  const maxVal = readProp<number | ''>(component, 'max', '');
  ...
  return (
    <div>
      ...
      <input
        ...
        placeholder={hasBoundField ? '' : (customPlaceholder || '0')}
        required={isRequired}
        min={minVal !== '' ? minVal : undefined}
        max={maxVal !== '' ? maxVal : undefined}
        className="w-full rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-4 py-2.5 text-sm text-zinc-700 dark:text-zinc-300 placeholder:text-zinc-400"
      />
    </div>
  );
}
