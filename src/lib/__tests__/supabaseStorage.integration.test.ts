 ...
 *
 * Meta rows use per-session keys (`{persistName}:user:{auth.uid()}`). RLS requires a JWT,
 * so set `SUPABASE_INTEGRATION_EMAIL` and `SUPABASE_INTEGRATION_PASSWORD` for these tests.
 *
 * Run with: npx jest src/lib/__tests__/supabaseStorage.integration.test.ts
 */
...
/** Match `PERSIST_VERSION` in `src/lib/supabaseStorage.ts` / `version` in projectStore persist. */
const PERSIST_VERSION = 8;

const integrationAuthConfigured =
  Boolean(process.env.SUPABASE_INTEGRATION_EMAIL) &&
  Boolean(process.env.SUPABASE_INTEGRATION_PASSWORD);
...
describe("supabaseStorage", () => {
  ...
  skipIfNoSupabase("with Supabase configured", () => {
    ...
    const describeWithSession = integrationAuthConfigured
      ? describe
      : describe.skip;
    ...
    describeWithSession("with signed-in user (RLS)", () => {
      let metaRowKey = `${TEST_KEY}:anonymous`;
      ...
      beforeAll(async () => {
        ...
        metaRowKey = `${TEST_KEY}:user:${user!.id}`;
      }, 30_000);

      afterAll(async () => {
        const keysToClear = [metaRowKey, TEST_KEY, `${TEST_KEY}:anonymous`];
        for (const k of keysToClear) {
          ...
          await supabaseClient.from("store_state_meta").delete().eq("key", k);
        }
