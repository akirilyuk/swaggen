...
- do not create empty catch blocks, if you need to catch an error, make sure to handle it properly by logging it or rethrowing it with additional context. This helps to improve the observability of the application and makes it easier to debug issues when they arise. Empty catch blocks can hide errors and make it difficult to identify and fix problems in the code. also log the error with enough context to understand what went wrong and where, this can help to identify patterns and common issues that may arise in the application, and can also provide valuable information for debugging and troubleshooting when issues do occur.
